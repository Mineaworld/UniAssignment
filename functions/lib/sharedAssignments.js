...
exports.listUserAssignmentsWithReminders = listUserAssignmentsWithReminders;
...
async function listUserAssignmentsWithReminders(db, userUid) {
    const [personalAssignmentsSnapshot, sharedPointersSnapshot] = await Promise.all([
    ...
    const sharedGroups = await Promise.all(sharedPointersSnapshot.docs.map(async (sharedPointerDoc) => {
        ...
        const assignmentRefs = stateSnapshot.docs.map((stateDoc) => (db.doc(`sharedSpaces/${spaceId}/assignments/${stateDoc.id}`)));
