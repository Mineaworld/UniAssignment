...
// Default data for demonstration, can be overridden by props if needed in future
const defaultData = [
    { label: "Mon", value: 65 },
    { label: "Tue", value: 85 },
    { label: "Wed", value: 45 },
...
export function MiniChart({ data = defaultData, className, title = "Activity" }: MiniChartProps) {
    ...
    return (
        <div
            ...
            className={cn("group relative w-full h-full p-6 rounded-2xl bg-foreground/[0.02] border border-foreground/[0.06] backdrop-blur-sm transition-all duration-500 hover:bg-foreground/[0.04] hover:border-foreground/[0.1] flex flex-col gap-4", className)}
        >
            {/* Header */}
            ...
            {/* Chart */}
            <div className="flex items-end gap-2 h-full min-h-[5rem]">
                {data.map((item, index) => {
                    const heightPx = (item.value / maxValue) * 100 // Use percentage for height logic
                    ...
                    return (
                        ...
                        >
                            ...
                            <span
                                className={cn(
                                    "text-[10px] font-medium mt-2 transition-all duration-300 text-center leading-tight break-words w-full",
