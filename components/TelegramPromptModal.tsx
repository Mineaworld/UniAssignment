...
const TelegramPromptModal: React.FC<TelegramPromptModalProps> = ({ isOpen, onClose, userUid }) => {
  ...
  const handleLinkTelegram = async () => {
    setIsLinking(true);
    try {
      const url = await generateTelegramLinkUrl(userUid);
...
<button
                    ...
                    disabled={isLinking}
                    className="w-full flex items-center justify-center gap-2 bg-[#0088cc] hover:bg-[#0077b5] disabled:opacity-50 disabled:cursor-not-allowed text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-[#0088cc]/20 hover:shadow-xl hover:-translate-y-0.5"
                  >
                    {isLinking ? (
                    ...
                    {isLinking ? 'Generating Link...' : 'Link Telegram Account'}
                  </button>
